...
<!-- _backgroundImage: "linear-gradient(135deg, #0b2b5e, #1a3a7a)" -->
<!-- _color: white -->
...
```mermaid
graph TB
    subgraph "<<device>> Docker Host"
        subgraph "<<execution environment>> Node.js Runtime"
            FE["<<artifact>> React Frontend<br/>(Port 5173)"]
        end
        subgraph "<<execution environment>> JRE 17"
            BM["<<artifact>> backend-main<br/>(Port 8080)"]
            BA["<<artifact>> backend-asta<br/>(Port 8081)"]
        end
        subgraph "<<execution environment>> DB Server"
            PG["<<artifact>> PostgreSQL 16<br/>(Port 5432)"]
        end
        subgraph "<<execution environment>> Message Broker"
            MQ["<<artifact>> Mosquitto<br/>(Port 1883)"]
        end
        subgraph "<<execution environment>> File System"
            VOL["<<artifact>> uploads/"]
        end
    end

    FE -. "<<protocol>> HTTP/REST" .-> BM
    BM -. "<<protocol>> JDBC" .-> PG
    BM -. "<<protocol>> MQTT (QoS 1)" .-> MQ
    BA -. "<<protocol>> MQTT (QoS 1)" .-> MQ
    BM -. "<<protocol>> File I/O" .-> VOL
```
...
classDiagram
    ...
    class Review {
        <<JPA Entity>>
        +Denormalized helpfulCount
    }

    User "1" --> "0..*" Event : hosts
    Event "1" --> "0..*" Rsvp : has
    Event "1" --> "0..*" Review : receives
    Event "1" --> "0..*" Category : categorized
```
...
classDiagram
    class EventFactory {
        -EventMessageTarget eventAdapter
        +createEvent(...) : Event
        +createDraft(...) : Event
...
classDiagram
    class EventMessageTarget {
        <<interface>>
        +adapt(OfficialEventMessage) Event
    }
...
classDiagram
    class RsvpEventPublisher {
        +publishRsvpCancelled(eventId, rsvpId)
    }
    ...
    class WaitlistPromotionListener {
        +handleRsvpCancelled(RsvpCancelledEvent)
    }

    RsvpEventPublisher ..> RsvpCancelledEvent : fires
    WaitlistPromotionListener ..> RsvpCancelledEvent : observes
```
...
<!--
  ================================================================
  APPENDIX – Q&A CHEAT SHEET (FOR SPEAKER PREPARATION)
  Do not present this slide; keep it for private rehearsal.
  ================================================================
-->
...
<!-- _footer: "✅ Team ready – Final presentation approved." -->